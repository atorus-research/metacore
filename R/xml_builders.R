...
xml_to_ds_vars <- function(doc) {
  ...
  xml_find_all(doc, "//ItemGroupDef") %>%
    ...
    select(
      .data$dataset, .data$variable, .data$key_seq,
...
xml_to_var_spec <- function(doc) {
  ...
  dist_df %>%
    ...
    select(
      .data$variable, .data$type, .data$length, .data$label,
...
xml_to_value_spec <- function(doc) {
  ...
  if (nrow(where_to_merge) == 0) {
    ...
    where_eqs <- full_join(where_to_merge, where_eqs, by = "where_oid") %>%
      ...
      mutate(
        oid = paste0("IT", str_remove(.data$oid, "^VL")),
  ...
  all_where_eqs <- where_eqs %>%
    ...
    ungroup() %>%
    select(-.data$eq,
      where = .data$full_eq, .data$derivation_id,
...
xml_to_codelist <- function(doc) {
  cl_pv <- xml_find_all(doc, "//CodeList") %>%
    map_dfr(function(node) {
      ...
      decodes <- c(
        xml_find_all(node_children_EI, "./Decode") %>% xml_text(),
      ...
      tibble(
        ...
        code = c(
          xml_attr(node_children_EI, "CodedValue"),
