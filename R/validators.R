  ...
  map2_lgl(tbl_name, tbls, function(name, tbl) {
    ...
    if (is.null(tbl)) {
      ...
      print_message <- name %>%
        str_remove("[:punct:]") %>%
        paste0(
          "'", ., "' has incorrect column names. It should be:\n",
...
all_message <- function() {
  tribble(
    ...
    "value_spec", "origin", function(x) {
      str_detect(x, "collected|derived|assigned|protocol|predecessor|crf.*") || is.na(x)
    }, TRUE,
