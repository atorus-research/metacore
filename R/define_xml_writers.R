...
metacore_to_define_xml <- function(
  ...
  if (is.null(study_details) || !is.list(study_details)) {
...
#'
#' `value_spec$where` holds an R expression string (e.g. `QNAM == "TRTEMFL"` or
#' `AVISIT %in% c("Week 8", "Week 16")`). This parses it into a list of range
#' checks, each `list(var, comparator, values)`, where `comparator` uses the
#' Define.xml vocabulary (`EQ`, `NE`, `LT`, `LE`, `GT`, `GE`, `IN`, `NOTIN`).
#' `AND` (`&`/`&&`) conditions yield multiple range checks. Unsupported
#' expressions yield an empty list (the caller then omits the where metadata).
#' @noRd
  ...
  for (grp in groups) {
    ...
    for (i in seq_len(nrow(grp))) {
      ...
      set_attrs(
        ...
        "OrderNumber" = i,
        "Mandatory" = "No",
        "MethodOID" = if (identical(origin_type(row$origin), "Derived")) row$derivation_id else NULL
      )
      wc_oid <- .generate_oid(
  ...
  for (d in seq_len(nrow(ds_spec))) {
    ...
    for (v in seq_len(nrow(vars))) {
      ...
      if (nrow(base_row) > 0 && identical(origin_type(base_row$origin[1]), "Derived")) {
        method_oid <- base_row$derivation_id[1]
      }
...
.create_item_defs <- function(mdv, metacore) {
  ...
  for (i in seq_len(nrow(ds_vars))) {
    ...
    comment_oid <- if (!is.null(base_row)) base_row$comment_id else NULL
    set_attrs(
      idef,
...
.create_comment_defs <- function(mdv, metacore) {
  ...
  for (i in seq_len(nrow(comments))) {
    ...
    cd <- xml2::xml_add_child(mdv, "def:CommentDef", OID = row$comment_id)
