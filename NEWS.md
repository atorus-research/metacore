...
-   `verbose` argument added to `metacore`, `spec_to_metacore`, `define_to_metacore`, and `select_dataset` to expand the user's ability to selectively suppress messages and warnings [#135](https://github.com/atorus-research/metacore/pull/138). Permits the options:
    -   **silent**: Suppresses both messages and warnings.
    -   **collapse**: Warnings are collapsed into a single message that indicates the number of warnings suppressed.
