...
test_that("values_spec reader tests", {
  ...
  spec_value_spec <- spec_type_to_value_spec(spec) %>%
    ...
    mutate(
      code_id = if_else(!is.na(code_id), paste0("CL.", code_id), code_id, NA_character_),
...
test_that("derivation reader tests", {
  ...
  ref_deriv <- spec$Variables %>%
    ...
    mutate(
      derivation_id = paste0("MT.", Dataset, ".", Variable),
      derivation = Description
    ) %>%
...
test_that("codelist reader tests", {
  ...
  ref_codelist <- tibble::tribble(
    ...
    "CL.VISIT", "VISIT", tibble(code = c(
      ...
      "WEEK 16", "WEEK 18 (T)", "UNSCHEDULED 10.2", "WEEK 20", "WEEK 22 (T)", "UNSCHEDULED 11.2", "WEEK 24", "UNSCHEDULED 12.1", "WEEK 26", "UNSCHEDULED 13.1", "AE FOLLOW-UP", "RETRIEVAL", "Rash followup"
    )), "code_decode",
    "CL.VISITNUM", "VISITNUM", tibble(code = c("1", "1.1", "1.2", "1.3", "2", "3", "3.1", "3.5", "4", "4.1", "4.2", "5", "5.1", "6", "6.1", "7", "7.1", "8", "8.1", "8.2", "9", "9.1", "9.2", "9.3", "10", "10.1", "10.2", "11", "11.1", "11.2", "12", "12.1", "13", "13.1", "101", "201", "501"), decode = c(
...
test_that("Check no value reader", {
  empty_val_check <- spec_to_metacore("spec_no_val.xlsx",
    verbose = "silent",
